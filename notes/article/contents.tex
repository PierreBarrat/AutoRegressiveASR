...
This has the advantage of greatly simplifying the analytical and numerical application, making models computationally manageable. 
...
An important application is that of ancestral sequence reconstruction (ASR): given a set of homologous sequences and their phylogenetic tree, ASR consists in inferring likely sequences for the internal nodes of the tree, which correspond to extinct ancestral proteins. 
Reconstructed proteins can then be synthesized and tested in the lab. 
The technique is used to study the sequence-function relationship for instance to understand which mutations cause a change in
...
It can also be used to address evolutionary questions, such as the evolution of fundamental characteristics of proteins across the tree of life \cite{akanuma_experimentalevidencethermophilicity_2013,hobbs_originevolutionthermophily_2012}. 
...
In practice, generative models are trained using large amounts of protein sequences and consist of a probability distribution $P(\mathbf{s})$ over any potential amino acid sequence. 
...
An essential feature for the success of generative model is the modeling of \emph{epistasis}: the fact that the effect of a mutation depends on the rest of the sequence. 
...
Since the change of a protein sequence in time depends on functional constraints, it is reasonable to expect that an inaccurate representation of the fitness landscape negatively affects the modeling of dynamics. 
...
Different groups have proposed using the Potts model to describe evolutionary dynamics, but current techniques allow for little analytical treatment and are limited to forward simulation of sequences \cite{alvarez_vivofunctionalphenotypes_2024,bari_emergenttimescales_2024}. 
...
This model accounts for epistasis and is generative over long-term evolution, but also allows use of some of the standard algorithms used in phylogenetics such as Felsenstein's pruning algorithm \cite{felsensteinjoseph_inferringphylogenies_2003}. 
...
Most commonly, $\mathbf{\Pi}$ is considered to be independent of the sequence position $i$, while $\mathbf{H}$ can be multiplied by position-dependent rates in order to model the different variability of different sites \cite{yang_maximumlikelihoodphylogenetic_1994}. \\
